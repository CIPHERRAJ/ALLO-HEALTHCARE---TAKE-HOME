        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 lg:p-12 overflow-hidden">
          ...
          <div className="relative w-full max-w-5xl max-h-[90vh] bg-white rounded-[48px] lg:rounded-[56px] shadow-[0_40px_150px_-20px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300 pointer-events-auto">
            ...
            <div className="flex flex-col lg:flex-row h-full">
               ...
               <div className="lg:w-[38%] bg-slate-900 text-white p-10 lg:p-14 flex flex-col justify-between relative overflow-hidden shrink-0">
                  ...
                  <div className="relative z-10 space-y-8">
                     ...
                     <Button 
                       ...
                       className="w-full h-16 bg-blue-600 hover:bg-blue-700 text-white rounded-[24px] font-black uppercase tracking-[0.2em] text-xs shadow-2xl shadow-blue-500/30 transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3"
                     >
                       {isReserving ? (
               ...
               <div className="flex-grow flex flex-col bg-white">
                  ...
                  <div className="flex-grow overflow-y-auto p-8 lg:p-10 space-y-6 scrollbar-hide bg-white">
                     {cartItems.length === 0 ? (
                       ...
                       cartItems.map((item) => (
                         <div key={`${item.productId}-${item.warehouseId}`} className="group flex items-center gap-8 bg-slate-50/50 p-6 lg:p-8 rounded-[40px] border border-slate-100/50 hover:bg-white hover:border-blue-100 hover:shadow-[0_20px_50px_-15px_rgba(0,0,0,0.06)] transition-all duration-500">
                           <div className="h-16 w-16 bg-white rounded-3xl flex items-center justify-center shrink-0 shadow-sm border border-slate-100 group-hover:scale-105 transition-transform duration-500">
                              <Package className="h-8 w-8 text-slate-400 group-hover:text-blue-500" />
                           </div>
                           
                           <div className="flex-grow min-w-0">
                              <div className="flex justify-between items-start mb-1">
                                 <h4 className="text-lg font-black text-slate-900 tracking-tight">{item.productName}</h4>
                                 <Button 
                                   variant="ghost" 
                                   size="icon" 
                                   onClick={() => removeFromCart(item.productId, item.warehouseId)}
                                   className="text-slate-200 hover:text-red-500 h-8 w-8 rounded-xl transition-colors -mr-2"
                                 >
                           ...
                           <div className="text-right shrink-0 ml-4">
                              <p className="text-xl font-black text-slate-900 tracking-tighter">${(item.price * item.units).toLocaleString()}</p>
                           </div>
                         </div>
                       ))
                     )}
                  ...
                  <div className="p-8 lg:p-10 border-t border-slate-50 flex items-center justify-between shrink-0 bg-slate-50/20">
                     <button 
                       onClick={() => clearCart()}
